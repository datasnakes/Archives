...
myblast = OrthoBlastN(proj_mana=None, project="sdh-test",
                      project_path=os.getcwd(), **Blast_config)
...
class BlastPipelineTask(SGEPipelineTask):
    ...
    def run(self):  # Use work instead of run to DEBUG
        myblast.blast_config(myblast.blast_human, 'Homo_sapiens',
                             auto_start=True)

    def output(self):
        return luigi.LocalTarget(path=os.path.join(self.path,
                                                   myblast.project_path.as_posix()))
